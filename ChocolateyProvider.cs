...
namespace Microsoft.PackageManagement.ChocolateyPrototype {
    ...
    public class ChocolateyProvider {
        ...
        public void AddPackageSource(string name, string location, bool trusted, ChocolateyRequest request) {
            string name1 = name;
            // Nice-to-have put a debug message in that tells what's going on.
            request.Debug("Calling '{0}::AddPackageSource' '{1}','{2}','{3}'", PackageProviderName, name1, location,
                trusted);
            ...
            if (src != null && !isUpdate) {
                ...
                request.Error(ErrorCategory.InvalidArgument, name1 ?? location, Constants.Messages.PackageSourceExists,
                    name1 ?? location);
                // we're done here.
                return;
            }
            ...
            if (src == null && isUpdate) {
                ...
                request.Error(ErrorCategory.ObjectNotFound, name1 ?? location, Constants.Messages.UnableToResolveSource,
                    name1 ?? location);
                // we're done here.
                return;
            }
            ...
            if (!request.SkipValidate) {
                ...
                if (!validated) {
                    request.Error(ErrorCategory.InvalidData, name1 ?? location, Constants.Messages.SourceLocationNotValid,
                        location);
                    // we're done here.
                    return;
                }
        ...
        public void FindPackage(string name, string requiredVersion, string minimumVersion, string maximumVersion,
            ...
            request.Debug("Calling '{0}::FindPackage' '{1}','{2}','{3}','{4}','{5}'", PackageProviderName, name,
                requiredVersion1, minimumVersion1, maximumVersion1, id);
            ...
            if (!IsValidVersionRange(minimumVersion1, maximumVersion1)) {
                request.Error(ErrorCategory.InvalidArgument, minimumVersion1 + maximumVersion1,
                    Constants.Messages.InvalidVersionRange, minimumVersion1, maximumVersion1);
                return;
            }
            ...
            if (!string.IsNullOrWhiteSpace(name)) {
                if (
                    request.YieldPackages(
                        request.GetPackageById(name, requiredVersion1, minimumVersion1, maximumVersion1), name) ||
        ...
        ///     the CORE and HOST
        /// </param>
        public void GetInstalledPackages(string name, string requiredVersion, string minimumVersion,
            string maximumVersion, ChocolateyRequest request) {
            string minimumVersion1 = minimumVersion;
            string maximumVersion1 = maximumVersion;
            // Nice-to-have put a debug message in that tells what's going on.
            request.Debug("Calling '{0}::GetInstalledPackages' '{1}','{2}','{3}','{4}'", PackageProviderName, name,
                requiredVersion, minimumVersion1, maximumVersion1);
                    ...
                    if (pkgItem != null && pkgItem.IsInstalled) {
                        if (pkgItem.Id.Equals(name, StringComparison.CurrentCultureIgnoreCase)) {
                            if (!string.IsNullOrWhiteSpace(requiredVersion)) {
                                ...
                                if (!string.IsNullOrWhiteSpace(minimumVersion1) &&
                                    pkgItem.Package.Version < new SemanticVersion(minimumVersion1)) {
                                    continue;
                                }
                                if (!string.IsNullOrWhiteSpace(maximumVersion1) &&
                                    pkgItem.Package.Version < new SemanticVersion(maximumVersion1)) {
                                    continue;
                                }
                            }
